(() => {
  ...
  const state = {
    ...
    missed: new Set(JSON.parse(storage.get('ielts538.missed') || '[]')),
  ...
  function setView(view) {
    if (!['card', 'list', 'review'].includes(view)) return;
    state.view = view;
  ...
  function setCategory(category) {
    ...
    state.cardIndex = 0;
    storage.set('ielts538.category', category);
    document.querySelectorAll('.scope-btn').forEach((button) => button.classList.toggle('active', button.dataset.category === category));
  ...
  function renderCard() {
    ...
    if (!list.length) {
      ...
      $('card-term').textContent = '没有匹配条目';
      $('card-meaning').textContent = '换一个搜索词或筛选范围。';
    ...
    $('card-source-page').textContent = `PDF p.${entry.sourcePage}`;
    ...
    $('source-term').textContent = entry.sourceTerm;
    ...
    $('card-progress-sub').textContent = state.auditOnly
      ? `疑点筛选 · ${list.length} 条`
  ...
  function moveCard(delta) {
    ...
    state.cardIndex = Math.max(0, Math.min(list.length - 1, state.cardIndex + delta));
    renderCard();
    const entry = list[state.cardIndex];
  ...
  function renderList() {
    ...
    if (!list.length) { $('word-list').innerHTML = '<div class="empty-state">没有匹配条目。</div>'; return; }
    $('word-list').innerHTML = list.map((entry) => `
      <button class="list-row" type="button" data-entry-id="${esc(entry.id)}">
        <div class="list-term">
          ...
          <div class="list-meta"><span>${esc(categoryLabel(entry))}</span><span>PDF p.${entry.sourcePage}</span></div>
        </div>
        <div class="list-meaning">${esc(entry.meaning || '—')}</div>
        <div class="list-mapping">${esc(entry.sourceMapping || '—')}</div>
  ...
  function restartReview(forceShuffle = true) {
    const list = filteredEntries();
    if (forceShuffle || !state.reviewQueue.length || state.reviewQueue.some((id) => !list.some((e) => e.id === id))) {
      state.reviewQueue = shuffle(list.map((entry) => entry.id));
      state.reviewIndex = 0;
    } else {
  ...
  function renderReview() {
    ...
    if (!has) {
      ...
      $('review-question').textContent = '没有可复习条目';
      $('review-answer-wrap').classList.add('hidden');
      $('reveal-answer').classList.add('hidden');
      $('review-actions').classList.add('hidden');
      $('review-progress').textContent = '调整搜索或筛选范围后再试。';
      return;
    }

    $('reveal-answer').classList.remove('hidden');
    $('review-kicker').textContent = `${categoryLabel(entry)} · PDF p.${entry.sourcePage}${state.missed.has(entry.id) ? ' · 错词' : ''}`;
    ...
    $('review-mapping').textContent = entry.sourceMapping ? `原书关联表达：${entry.sourceMapping}` : '原书未列关联表达';
  ...
  function markReview(known) {
    ...
    if (state.reviewIndex >= state.reviewQueue.length) {
      state.reviewQueue = shuffle(state.reviewQueue);
      state.reviewIndex = 0;
    }
    state.reviewRevealed = false;
    renderReview();
  ...
  function bindEvents() {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (state.view === 'review' && event.code === 'Space') { event.preventDefault(); if (!state.reviewRevealed) revealReview(); }
    });
  }
  ...
  function init() {
    ...
    $('audit-count').textContent = `${meta.auditCount || 0} 条已标疑点`;
    $('rate-value').textContent = `${state.speechRate.toFixed(2)}×`;
